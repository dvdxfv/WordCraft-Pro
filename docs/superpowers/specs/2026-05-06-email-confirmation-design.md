...
3. **区分处理不同状态** — 验证成功、重复点击验证链接等情况显示不同的提示和操作指南
...
### 3. 错误处理：区分状态
- 加载状态：验证中的加载动画
- 成功：显示“验证成功”、邮箱和时间，不自动跳转
- 链接异常：显示重发入口
- 已验证：显示“显示已验证提示”，不进行跳转
...
| 场景 | 预期结果 |
|------|---------|
| 首次点击有效链接 | 显示成功页面 + 邮箱和时间，不跳转 |
| 24小时后点击链接 | 显示链接异常/重发入口 |
| 已验证邮箱再点链接 | 显示"显示已验证提示"，无跳转 |
...
- 实际实现取消了成功后的自动跳转，成功页只显示“验证成功”。
- Supabase 对"已使用链接"和"过期链接"返回相同 `error_code=otp_expired`，当前实现统一显示“显示已验证提示”，这是产品文案统一，不代表底层状态可被可靠区分。
