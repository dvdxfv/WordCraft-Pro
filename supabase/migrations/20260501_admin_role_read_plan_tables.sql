-- Batch 19: allow admin dashboard to read plan and team related tables.
...
begin
  ...
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'activation_codes'
      and policyname = 'admin_can_view_all_activation_codes'
  ) then
    create policy admin_can_view_all_activation_codes on public.activation_codes
      for select to authenticated
      using ((select auth.jwt() ->> 'role') = 'admin');
  end if;
